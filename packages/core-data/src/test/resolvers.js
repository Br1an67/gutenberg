...
describe( 'getRevisions', () => {
	...
	it( 'acquires and releases lock at the revisions path', async () => {
		triggerFetch.mockResolvedValue( REVISIONS );

		await getRevisions( kind, name, recordKey, { context: 'edit' } )( {
			dispatch,
			registry,
			resolveSelect,
		} );
	...
	it( 'releases lock on API error', async () => {
		triggerFetch.mockRejectedValue( new Error( 'API Error' ) );
	...
	it( 'calls finishResolutions with normalized query (context/_fields only)', async () => {
		triggerFetch.mockResolvedValue( REVISIONS );

		await getRevisions( kind, name, recordKey, {
			context: 'edit',
			per_page: 10,
		} )( {
			dispatch,
		...
		expect( dispatch.finishResolutions ).toHaveBeenCalledWith(
			'getRevision',
			[
				[ kind, name, recordKey, 1, { context: 'edit' } ],
				[ kind, name, recordKey, 2, { context: 'edit' } ],
				[ kind, name, recordKey, 3, { context: 'edit' } ],
			]
		);
	} );

	it( 'calls finishResolutions with undefined when query has no relevant keys', async () => {
		triggerFetch.mockResolvedValue( REVISIONS );

		await getRevisions( kind, name, recordKey, { per_page: 10 } )( {
			dispatch,
			registry,
			resolveSelect,
		} );
		...
		expect( dispatch.finishResolutions ).toHaveBeenCalledWith(
			'getRevision',
			[
				[ kind, name, recordKey, 1, undefined ],
				[ kind, name, recordKey, 2, undefined ],
				[ kind, name, recordKey, 3, undefined ],
			]
		);
	} );

	it( 'calls receiveRevisions with the full original query, not the normalized one', async () => {
		triggerFetch.mockResolvedValue( REVISIONS );
		const originalQuery = { context: 'edit', per_page: 10 };
		...
		expect( dispatch.receiveRevisions ).toHaveBeenCalledWith(
			kind,
			name,
			recordKey,
			expect.any( Array ),
...
describe( 'getRevision', () => {
	...
	const REVISION = { id: revisionKey, title: 'Rev 5' };
	const resolveSelect = { getEntitiesConfig: jest.fn( () => ENTITIES ) };
	...
	beforeEach( () => {
		select = {
			getRevision: jest.fn().mockReturnValue( undefined ),
		};
		dispatch = Object.assign( jest.fn(), {
			receiveRevisions: jest.fn(),
			__unstableAcquireStoreLock: jest.fn().mockResolvedValue( 'lock' ),
	...
	it( 'acquires and releases lock at the revision path', async () => {
		triggerFetch.mockResolvedValue( REVISION );
	...
	it( 'skips API fetch when cached revision found inside lock', async () => {
		select.getRevision.mockReturnValue( REVISION );
		...
		expect( dispatch.receiveRevisions ).not.toHaveBeenCalled();
		expect( dispatch.__unstableReleaseStoreLock ).toHaveBeenCalledWith(
			'lock'
		);
	} );

	it( 'fetches from API and calls receiveRevisions when revision not in cache', async () => {
		triggerFetch.mockResolvedValue( REVISION );
		...
		expect( triggerFetch ).toHaveBeenCalledWith(
			expect.objectContaining( {
				path: expect.stringContaining(
					`/wp/v2/posts/${ recordKey }/revisions/${ revisionKey }`
				),
			} )
		);
		expect( dispatch.receiveRevisions ).toHaveBeenCalledWith(
	...
	it( 'releases lock on API error without calling receiveRevisions', async () => {
		triggerFetch.mockRejectedValue( new Error( 'API Error' ) );
