...
-   **Origin**: A value attached to each Yjs transaction to identify the source of a change (e.g., local editor, sync manager, or remote peer). Origins are used to decide which changes should trigger store updates and which should be tracked by the undo manager.
...
The sync config "owns" the sync behavior of the entity; it has sole knowledge of the entity schema and edit flows. In Gutenberg, the sync config is defined and controlled by the `core-data` package.
